...
| Table 3 / Table 4 samples | 19,965 challengers / 17,336 debates; Table 4 has 2,784 challenger clusters |
...
| Table 3 underlying model output | All 35 coefficients, standard errors, test statistics, and p-values reproduce within numeric tolerance |
| Table 4 | Exact text match with archived no-control empirical-claim table, including the manuscript's displayed Table 4 values |
| Engagement indicator tests supporting Figures 2–3 | All 273 saved tests reproduce: group counts, means, differences, U statistics, and p-values |
| Engagement text-feature tests | All 294 saved tests reproduce using round-trip float parsing |
| Figures 2 and 3 | Pixel-identical to the actual embedded images on PDF pages 8 and 9, as well as the saved source images |
| Moderation robustness (supplemental) | Topic FE, weighting, CEM, alternative moderators, and poster-control scripts complete; aggregate numeric outputs are checked against saved reference CSVs |
...
For item-by-item coverage and the exact Table 3 discrepancies, see [Results indexed to the HICSS paper](PAPER_RESULTS.md). Pixel checks compare RGB values of the embedded PDF images with the regenerated PNGs; the PDF captions are outside those images.
